...
function game(e){
    ...
    if (ignoreCasing.checked){
        if (e.data.toLowerCase() === currentChar.innerText.toLowerCase()) {
        ...
        if (e.data === currentChar.innerText){
            currentChar.classList.remove('highlighted');
            currentChar.classList.add('correct');
            ++correctChars
        } else {
...
function startGame () {
    ...
    typeBar.focus()
    typeBar.addEventListener('input', keyListener);
}
...
function keyListener(e){
    ...
    // Call game function if the event contains a character, number or special character
    /*var inp = String.fromCharCode(e.key);
...
function stopGame(){
    ...
    typeBar.removeEventListener('input', keyListener)
    startTime = null;
}
